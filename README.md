...
用于领物 TEMU 流程自动化：上传图库、批量设计、成品汇出。当前版本：`v2.1.10`。

## 功能

- 上传图库：批量读取图片文件夹，统一设置标签，默认标签精确到秒，单张失败自动重试 1 次。
- 失败处理：第一步仍失败的图片会复制到原文件夹下的 `上传失败_标签_店铺_公版` 目录。
- 文件夹标记：第一步全成功后，原图片文件夹自动追加 `（已上传图库）`。
- 批量设计：按标签选择主题图，按公版名称精确匹配设计公版，支持“最大化设计”选项，接口异常自动重试 1 次。
- TEMU 汇出：按标签筛选成品，支持单店铺或多店铺汇出，按店铺自动生成默认 SKC，接口异常自动重试 1 次。
- 队列任务：支持任务排队，运行中新增队列会在当前任务结束后自动续跑。
- 错误展示：三步流程中后续步骤失败时，会保留已完成步骤的真实结果；第二步/第三步前置失败也会写失败报告，不再只显示子进程退出码。

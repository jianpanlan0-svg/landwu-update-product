...
function updateActionButtons(running, queueHasItems) {
  startPipelineBtn.disabled = !!running;
  runStep1Btn.disabled = !!running;
  runStep2Btn.disabled = !!running;
  runStep3Btn.disabled = !!running;
  pickFolderBtn.disabled = !!running;
  stopBtn.disabled = !running;
  runQueueBtn.disabled = !queueHasItems;
}
...
function renderStatus(payload) {
  ...
  if (!running && lastResult?.finishedAt && lastResult.finishedAt !== lastTaskFinishedAt) {
    ...
    if (lastResult.status === 'success') {
      ...
      setMessage(reportText ? `${base} | 报告：${reportText}` : base);
    } else if (lastResult.status === 'error') {
      setMessage(reportText ? `${lastResult.message} | 报告：${reportText}` : lastResult.message || '任务失败', true);
...
function getTaskTitle(task) {
  ...
  return `${task.folderPath || '单步任务'} | ${task.tag || '自动标签'} | ${steps}`;
}
...
function renderQueue() {
  ...
  queueSummary.textContent = `队列任务：排队 ${waiting} | 执行中 ${doing} | 完成 ${done} | 失败 ${failed}`;

  if (!queue.length) {
    queueSummary.textContent = '说明：下方“立即执行当前任务”是直接跑当前表单；这里只负责排队任务';
    queueList.innerHTML = '<div class="queue-empty">暂无排队任务。要批量排队时，先点上方“加入任务队列”。</div>';
      ...
      <div class="queue-item">
        <div class="queue-item-top">
          ...
          <div class="queue-item-actions">
            <div class="queue-status is-${escapeHtml(item.status)}">${escapeHtml(item.status)}</div>
            ${deleteButton}
          </div>
        </div>
...
async function runStep2Only() {
  ...
  return postTask('/api/start-step2', { tag: payload.tag, templateName: payload.designTemplateName }, '第2步已启动');
}
...
async function runQueue() {
  ...
  if (!hasQueued && !hasRunning) {
    setMessage('队列里还没有任务，请先点“加入任务队列”', true);
    renderQueue();
    return;
  }
  queueRunning = true;
...
async function tickQueue(statusData) {
  ...
  if (!queueRunning || statusData.running) return;

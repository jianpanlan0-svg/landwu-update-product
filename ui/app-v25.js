...
function getEntryStatus(status, result) {
  if (result?.ok === false || result?.failed) return 'error';
  if (Number(result?.failureCount || 0) > 0) return 'partial';
  if (status === 'error' && result && (Number(result.successCount || result.count || 0) > 0 || result.reportFile)) return 'success';
  return status;
}
...
function buildHistoryLine(stepKey, result, context = {}, status = 'success') {
  ...
  if (stepKey === 'step1') {
    ...
    if (status === 'error') {
      return `任务标签为 ${tag || '-'} 的上传图库失败，计划 ${totalCount} 张图${folderText}，${baseStatus}`;
    }
    return `把任务标签为 ${tag || '-'} 的 ${totalCount} 张图${folderText}上传到图库了，${baseStatus}`;
  }
  if (stepKey === 'step2') {
    if (status === 'error') {
      return `任务标签为 ${tag || '-'} 的批量设计失败，计划 ${totalCount} 张图，${baseStatus}`;
    }
    return `把任务标签为 ${tag || '-'} 的 ${totalCount} 张图设计为成品了，${baseStatus}`;
  }
  if (stepKey === 'step3') {
    if (status === 'error') {
      return `任务标签为 ${tag || '-'} 的 TEMU 汇出到 ${shopName || '-'} 店铺失败，计划 ${totalCount} 个成品，${baseStatus}`;
    }
...
function buildHistoryMeta(stepKey, result, context = {}) {
  ...
  if (stepKey === 'step1' && result?.folderMarkedPath) {
    parts.push(`已标记：${result.folderMarkedPath}`);
  } else if (stepKey === 'step1' && result?.folderMarkError) {
    parts.push(`标记失败：${result.folderMarkError}`);
  }
  if (result?.errorMessage) {
    parts.push(`失败原因：${result.errorMessage}`);
  } else if (Array.isArray(result?.failures)) {
    const firstFailure = result.failures.find((item) => item?.error);
    if (firstFailure?.error) parts.push(`失败原因：${firstFailure.error}`);
  }
  return parts.join(' | ');
}
...
function formatFriendlyResultText(type, result) {
  ...
  if (type === 'step1') {
    const markText = result.folderMarkedPath
      ? `，已标记文件夹`
      : (result.folderMarkError ? `，文件夹标记失败` : '');
    return failureCount
      ? `上传 ${successCount} 张图片，失败 ${failureCount} 张`
      : `上传 ${successCount} 张图片${markText}`;
  }
...
function renderResultPanel(lastResult, currentTask, logs = []) {
  ...
  if (status === 'error') {
    const summary = buildResultSummary(lastResult);
    const realError = getRealErrorMessage(lastResult.message, logs) || lastResult.message || '任务失败';
    resultPanelSummary.textContent = [summary, realError].filter(Boolean).join(' | ');
  } else if (status === 'stopped') {
    resultPanelSummary.textContent = '任务已手动停止';
  } else {
    const markDigest = buildFolderMarkDigest(lastResult);
    resultPanelSummary.textContent = [buildResultSummary(lastResult) || lastResult.result?.message || '任务完成', markDigest]
      .filter(Boolean)
      .join(' | ');
  }
...
function buildFolderMarkDigest(lastResult) {
  const result = lastResult?.result || {};
  const step1Result = result.stepResults?.step1 || (result.step === 'step1' || result.folderMarkedPath || result.folderMarkError ? result : null);
  if (!step1Result) return '';
  if (step1Result.folderMarkedPath) {
    return `已标记文件夹：${step1Result.folderMarkedPath}`;
  }
  if (step1Result.folderMarkError) {
    return `文件夹标记失败：${step1Result.folderMarkError}`;
  }
  return '';
}
...
function renderStatus(payload) {
  ...
  if (!running && lastResult?.finishedAt && lastResult.finishedAt !== lastTaskFinishedAt) {
    ...
    if (lastResult.status === 'success') {
      ...
      const folderMarkDigest = buildFolderMarkDigest(lastResult);
      if (folderMarkDigest) parts.push(folderMarkDigest);

...
function buildHistoryLine(stepKey, result, context = {}, status = 'success') {
  ...
  if (stepKey === 'step1') {
    return `把任务标签为 ${tag || '-'} 的 ${totalCount} 张图上传到图库了，${baseStatus}`;
  }
  if (stepKey === 'step2') {
...
function createHistoryEntries(lastResult, currentTask) {
  ...
  if (status === 'success' && result.stepResults) {
    ...
    if (result.stepResults.step1) {
      entries.push({
        ...
        time: finishedAt,
        status,
        statusText: getResultStatusText(status),
        text: buildHistoryLine('step1', result.stepResults.step1, baseContext, status),
        meta: `标签：${baseContext.tag || '-'}`,
      });
    }
    if (result.stepResults.step2) {
      entries.push({
        ...
        status,
        statusText: getResultStatusText(status),
        text: buildHistoryLine('step2', result.stepResults.step2, baseContext, status),
        meta: `标签：${baseContext.tag || '-'}`,
      });
    }
    if (result.stepResults.step3) {
      ...
      entries.push({
        ...
        status,
        statusText: getResultStatusText(status),
        text: buildHistoryLine('step3', result.stepResults.step3, { ...baseContext, shopName: finalShopName }, status),
        meta: `标签：${baseContext.tag || '-'} | 店铺：${finalShopName || '-'}`,
      });
    }
    return entries;
  }
  ...
  return [{
    ...
    status,
    statusText: getResultStatusText(status),
    text: buildHistoryLine(stepKey, result, baseContext, status),
    meta: stepKey === 'step3'
      ? `标签：${baseContext.tag || '-'} | 店铺：${baseContext.shopName || '-'}`
      : `标签：${baseContext.tag || '-'}`,
  }];
}
...
function saveTaskHistory() {
  localStorage.setItem(HISTORY_STORAGE_KEY, JSON.stringify(taskHistory.slice(0, 50)));
}
...
function addCurrentTaskToQueue() {
  ...
  renderQueue();
  if (runningText.textContent === '运行中') {
    setMessage(`已加入队列：${getTaskTitle(item)}；当前任务结束后会自动继续执行队列`);
    return;
  }
